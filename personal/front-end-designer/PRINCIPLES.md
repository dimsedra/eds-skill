...
3. Apply consistently — buttons, cards, inputs all wear the same material. A brand that feels weighty on its buttons but flimsy on its cards isn't materialized, it's decorated.

**Default stack flattening:** default font stacks (Inter, Roboto, Arial, raw `system-ui`) are the fast path to looking like everyone else — the brand's typography is part of its moat. If a system font is genuinely right for the brand, customize it (letter-spacing, stylistic sets) rather than defaulting.
